...
export default {
  ...
  methods:{
    Reload()
    {
      ...
      //调用后台函数，传递参数
      this.axios.get('/'+this.$WarmTable+'/alarm/getdutyinfo',{params:{begincount:_bgincount,endcount:_endcount,year:this.form.year,sign:this.form.dutytype,addtion1:this.form.searchmsg,orderBy:this.form.orderby,sequence:this.form.sequence}}).then((res)=>{
                    this.loading = false;
    ...
    exportData(){
      ...
      window.location.href='/'+this.$WarmTable+'/excel/exportdutyinfo?'+params;
    },
    // 处理页码切换
    CurrentChange(index) 
    {
    ...
    deleteClick(row){
      this.$confirm('是否删除该记录?', '提示', {
          ...
          this.axios.get('/'+this.$WarmTable+'/alarm/deldutyinfo',{params: {id:row.ID}}).then(res => {
          if(res.data=="ok"){
    ...
    getyears(){
      ...
      this.axios.get('/'+this.$WarmTable+'/alarm/getdutyyears')
                    .then(res => {
                        this.years = res.data;
    ...
    exportAddrlist(){
      if(this.form.yr==this.form.toyr){
        ...
        this.axios.get('/'+this.$WarmTable+'/alarm/existdata',{params: {yr:this.form.toyr}}).then(res => {
    ...
    //导入通讯录
    addAddrlist(sign){
      this.axios.get('/'+this.$WarmTable+'/alarm/addbyyear',{params: {yr:this.form.yr,toyr:this.form.toyr}}).then(res => {
          if(res.data=="ok"){
