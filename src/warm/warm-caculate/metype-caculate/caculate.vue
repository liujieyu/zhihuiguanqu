...
export default {
  data() {
    return {
        ...
        searchmsg:'',
        marginleft:window.innerWidth*19/24*0.5-150,
        Treedata: [],
      defaultProps: {
          children: 'children',
          label: 'STNM'
    ...
    onCaculate(meType){
       var regPos = /^\d+(\.\d+)?$/;
        ...
        this.axios.post('/'+this.$WarmTable+'/ldresult/getq',p_obj).then((res)=>{
          var q=res.data;
            if(q!=-1 && q!="NaN"){
              if(meType==1){
    ...
    onSave(meType){
      ...
      this.axios.get('/'+this.$WarmTable+'/ldresult/existresult',{params: s_obj}).then(res => {
          if(res.data==0){
            this.axios.post('/'+this.$WarmTable+'/ldresult/addeqrl',s_obj).then((res)=>{
            if(res.data=="ok"){
