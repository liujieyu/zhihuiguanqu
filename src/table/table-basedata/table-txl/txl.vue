                    ...
                    <Row type="flex" :gutter="16" justify="start">
                        ...
                        <Col>
                            ...
                            <Select v-model="lx.Field" style="width:120px;margin-left: 5px;margin-right: 20px;">
                                <Option v-for="item in lx.TYPES" :value="item.Field" :key="item.Field">{{ item.FieldName }}</Option>
                            </Select>
                        </Col>
                        <Col>
        ...
        methods:{
            ...
            search(){
              if (this.form.searchmsg != '') {
                this.searchs = '&NM=like,'+this.form.searchmsg;
              }else{
                this.searchs = '';
              }
                this.Reload();
            ...
            Reload(){
                this.loading = true;
                this.axios.get('/guanqu/admin/WRP_MAN?_page_size='+this.list_input.pagesize+'&_page='+this.list_input.current+this.searchs,{params:{TYPES: this.lx.TYPES}}).then((res)=>{
                    console.log(res);
                    this.loading = false;
                ...
                this.axios.get('/guanqu/admin/WRP_FieldInfo?FieldID=TYPES').then((res)=>{
                  this.lx.TYPES = res.data.rows;
                });
            },
            // 处理页码切换
            CurrentChange(index) {
