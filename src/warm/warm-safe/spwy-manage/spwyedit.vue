...
export default {
  ...
  data() {
    return {
      ...
      routerurl:'/guanqu/manage/addspwyinfo',
      stcdlist:[],
      damcdlist:[],
      ofaxfh:"+",
      rules: {
        ...
        stvlx: [{ required: true, message: "基准值X不能为空",trigger: 'blur'}],
        stvly: [{ required: true, message: "基准值Y不能为空",trigger: 'blur'}],
        el:[{ required: true, message: "初始高程不能为空",trigger: 'blur'}],
        ...
        xyhrds:[{ required: true, message: "位移阈值不能为空",trigger: 'blur'}],      
      },
    };
  },
  // 引入过滤方法到此组件
  ...
  methods: {
    ...
    Reload(){   
      if(this.info.editsign=="update"){
        this.routerurl='/guanqu/manage/updatespwyinfo';
        this.axios.get('/guanqu/manage/spwydetail',{params:{ID:this.info.id}}).then((res)=>{
          this.form=res.data;
          this.getDamcd(this.form.stcd);
        }); 
      }else{
        this.routerurl='/guanqu/manage/addspwyinfo';
      }
    },
    //获取断面编码列表
    getDamcd(stcd){
      ...
      this.$refs['slylform'].validate((valid) => {
          if (valid) {            
            if(this.info.editsign=="add" || (this.info.editsign=="update" && this.info.mpcd!=this.form.mpcd)){
              this.axios.get('/guanqu/base/checkmpcd',{params:{MPCD:this.form.mpcd,TYPE:3}}).then((res)=>{
