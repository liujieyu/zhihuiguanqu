    ...
    <div class="baseBoxContent">
        ...
        <div>
            ...
            >
                ...
                ></el-table-column>
                <el-table-column align="center" prop="vrds" label="垂直位移(mm)" sortable="custom" :width="125">
                    <template slot-scope="scope">
                        <div>
                            <span
                                :style="{
                            color: scope.row.vjyz >= 0?'red' : 'black',
                            }"
                            >{{ scope.row.vrds }}</span>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="vjyz" label="距阈值(mm)" sortable="custom" :width="115">
                    <template slot-scope="scope">
                        {{ scope.row.vjyz }}
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="inel" label="测量高程(m)" sortable="custom" :width="120">
                    <template slot-scope="scope">
                        {{ scope.row.inel }}
                    </template>
                </el-table-column>
        ...
        methods: {
            ...
            getTableDataFormParent() {
                ...
                this.featrue.Rows.map(val => {
                    ...
                    for(var i=0;i<rowinfo.czwylist.length;i++){
                        var rowobj=rowinfo.czwylist[i];
                        ...
                        dataobj.vrds=rowobj.vrds;
                        dataobj.inel=rowobj.inel;
                        dataobj.vjyz=rowobj.vjyz;
                        dataobj.vrdsyj=rowobj.vrdsyj;
                ...
                if (order) {
                    switch (order) {
                        ...
                        if(key=="STNM"){
                            ...
                            newList = this.table.Rows_filter.sort((a, b) => {
                                var coma=a[key].substr(2);
                                var comb=b[key].substr(2);
