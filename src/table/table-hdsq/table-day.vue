...
<!--                                <el-tag type="danger" v-if="scope.row.CV == '↑'">{{scope.row.CV}}</el-tag>-->
<!--                                <el-tag v-if="scope.row.CV == '↓'">{{scope.row.CV}}</el-tag>-->
<!--                                <el-tag type="info" v-if="scope.row.CV == '-'">{{scope.row.CV}}</el-tag>-->
              ...
              <el-dialog title="综合报表" :visible.sync="dialog_detail_showing" width="1200px" append-to-body>
                    ...
                    <Row>
                      <Col style='text-align: left;'>
                            总站数：{{Tables.total}}，单位：水位 m，流量 m³/s，累计流量 10⁶m³
                      </Col>
    ...
    export default {
        ...
        methods:{
            ...
            Reload(){
                ...
                this.axios.get('/guanqu/hedaoshuiqing/lishi_ribiao?_page_size='+this.list_input.pagesize+'&_page='+this.list_input.current,{params:{TM:this.form.date,ADDVCD:this.form.xzqh,Canal_Code:this.form.qd,STGR:this.form.zdlx,_orderby:this.form.field,STNM: this.searchs}}).then((res)=>{
                    console.log(res);
                    this.loading = false;
                ...
                // this.axios.get('/hedaoshuiqing/lishi_ribiao').then((res)=>{
                //   this.Tables.total = res.data.total;
                // });



                this.axios.get('/guanqu/hedaoshuiqing/lishi_ribiao?_page_size='+this.list_input.pagesize+'&_page='+this.list_input.current+this.searchs,{params:{TM:this.form.date,ADDVCD:this.form.xzqh,Canal_Code:this.form.qd,_orderby:this.form.field}}).then((res)=>{
                    console.log(res);
                    this.loading = false;
