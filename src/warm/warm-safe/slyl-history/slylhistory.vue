<template>
  <div>
      ...
      <Row :gutter="16">
        <Col :span="span1">
          <Row :gutter="16" type="flex" justify="start" z style="margin-top: 10px;line-height: 32px;">
            ...
            <Col>
              测点编号：
              <Select v-model="select.shenliuyali_select" @on-change="optionchange" style="width:130px;margin-right: 20px;" placeholder="测点编号" >
                                <Option v-for="item in select.slylmpsdList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                            </Select>
            </Col>
            <Col>
            <Button type="primary" style="width: auto;margin-right: 10px;" @click="SlylInfo">渗压测站信息</Button>
            </Col>
          </Row>
          ...
          <Row>
                <Col style='font-size: 14px;' class="borsLine">站名：{{slylInfo.stnm}}&nbsp;测点编号：{{slylInfo.mpcd}}&nbsp;渗压阈值：{{slylInfo.pztbtel}}m&nbsp;监测部位：{{slylInfo.msps}}&nbsp;
              单位：渗压水位 m，水温 ℃</Col>
            </Row>
   ...
   >
  <SLYLDETAIL v-if="detailitem.itemshow" :info="detailitem"></SLYLDETAIL>
  </el-dialog>
  </div>
</template>
...
import SLYLDETAIL from "@/warm/warm-safe/slyl-base/slyldetail.vue";
...
export default {
  ...
  mounted() {
    ...
    //获取行政区划树
    this.axios.get("/guanqu/info/xzqhtree?TYPE=2").then(res => {
  ...
  methods: {
    //弹框
    SlylInfo(){
      this.slldetail=this.slylInfo.stnm+"测站"+this.select.shenliuyali_select+"渗流压力信息";
              this.detailitem={mpcd:this.select.shenliuyali_select,itemshow:true};
    ...
    optionchange(){
      var stnm=this.slylInfo.stnm;
      this.list_input.current = 1;
      this.form.field = '';
       this.axios.get("/guanqu/detail/syginfo", {
            params: {MPCD:this.select.shenliuyali_select}
        }).then(res => {
            var data=res.data.syginfo;
            this.slylInfo=data[0];
            this.slylInfo.stnm=stnm;
            this.Reload();
        });         
    },
    search(){
        ...
        series: [
          {
            ...
            data: [{name:"渗压阈值"+this.data2.y1.markval,
    ...
    getMpcdList(callback){
      this.axios.get("/guanqu/info/cdbhlist?TYPE=2&STCD="+this.searchs).then(res => {
        this.select.slylmpsdList=res.data;
        this.select.shenliuyali_select=res.data[0].value;
        if (typeof callback == "function") {
          callback(this.select.shenliuyali_select);
        }
      });
    },
    ReloadBysort(){
      let obj = {
        ...
        MPCD: this.select.shenliuyali_select,
        ...
        data => {
          ...
          this.$GetData.Safe_History_DATA(
            ...
            chartData => {
              ...
              this.data2 = this.$App.transform_SAFE_data_into_ehart_data(this.data2,"slyl",true);             
              ...
              if(y1max<this.slylInfo.pztbtel){
                y1max=Math.ceil(this.slylInfo.pztbtel);
              }
              if(y1min>this.slylInfo.pztbtel){
                y1min=Math.floor(this.slylInfo.pztbtel);
              }
              this.data2.y1.max=y1max;
              this.data2.y1.min=y1min;
              this.data2.y1.markval=this.slylInfo.pztbtel;
