    ...
    <div class="baseBoxContent">
        ...
        <div>
            ...
            >
                ...
                ></el-table-column>
                <el-table-column align="center" prop="xhrds" label="X向位移(mm)" sortable="custom" :width="120">
                    <template slot-scope="scope">
                        <div>
                            <span
                                :style="{
                            color: scope.row.xjyz >= 0?'red' : 'black',
                            }"
                            >{{ scope.row.xhrds }}</span>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="yhrds" label="Y向位移(mm)" sortable="custom" :width="120">
                    <template slot-scope="scope">
                        <div>
                            <span
                                :style="{
                            color: scope.row.yjyz >= 0?'red' : 'black',
                            }"
                            >{{ scope.row.yhrds }}</span>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="xjyz" label="X距阈值(mm)" sortable="custom" :width="125">
                    <template slot-scope="scope">
                        {{ scope.row.xjyz }}
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="yjyz" label="Y距阈值(mm)" sortable="custom" :width="125">
                    <template slot-scope="scope">
                        {{ scope.row.yjyz }}
                    </template>
                </el-table-column>
        ...
        methods: {
            ...
            getTableDataFormParent() {
                ...
                this.featrue.Rows.map(val => {
                    ...
                    for(var i=0;i<rowinfo.spwylist.length;i++){
                        var rowobj=rowinfo.spwylist[i];
                        ...
                        dataobj.xhrds=rowobj.xhrds;
                        dataobj.yhrds=rowobj.yhrds;
                        dataobj.xyhrds=rowobj.xyhrds;
                        dataobj.xjyz=rowobj.xjyz;
                        dataobj.yjyz=rowobj.yjyz;
            ...
            sort_change(item) {
                ...
                if (order) {
                    switch (order) {
                        ...
                        }else if(key=="xhrds" || key=="yhrds"){
                            newList = this.table.Rows_filter.sort((a, b) => {
                                var coma=a[key].substr(3);
                                var comb=b[key].substr(3);
                                return Number(comb) - Number(coma);
                            });                           
                        } else if (key != "TM") {
                            newList = this.table.Rows_filter.sort((a, b) => {
                        ...
                        if(key=="STNM"){
                            ...
                            newList = this.table.Rows_filter.sort((a, b) => {
                                var coma=a[key].substr(3);
                                var comb=b[key].substr(3);
                                return Number(coma) - Number(comb);
                            });                           
                        }else if (key != "TM") {
                            newList = this.table.Rows_filter.sort((a, b) => {
