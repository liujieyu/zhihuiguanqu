...
<template>
  <div id="app">
    ...
    <div
      class="topNav"
      style="position: absolute; width: 100%; left:0; top: 0; z-index: 1; opacity: 0.9;display:none;"
    >
      <el-menu
        class="el-menu-demo"
      ...
      >
        ...
        <el-menu-item index="999" style="float: right;">
    ...
    <div v-show="mapBox_show == 1" id="mapBox">
      ...
      >
        ...
        <DetailSHUIKUSHUIQING
          ...
          v-if="details.info_right.itype=='shuikushuiqing'"
        ></DetailSHUIKUSHUIQING>
        <!-- 河道水情 -->
        <DetailHEDAOSHUIQING
          :info="details.info_right"
          v-if="details.info_right.itype=='hedaoshuiqing'"
        ></DetailHEDAOSHUIQING>
        <!-- 闸阀水情 -->
        <DetailZHAFASHUIQING
          :info="details.info_right"
        ...
        <DetailSHUIWEIKURONG
          :info="details.info_right"
          v-if="details.info_right.itype=='shuiweikurong'"
        ></DetailSHUIWEIKURONG>
        <!--站点预警详情-->
        <WarmSITEINFO :info="details.info_right" v-if="details.info_right.itype=='yujingxiangqing'"></WarmSITEINFO>
        <!--发送短信详情-->
        <WarmBOXINFO :info="details.info_right" v-if="details.info_right.itype=='fasongduanxin'"></WarmBOXINFO>
      </Drawer>
      ...
      >
        ...
        <DetailSHUIKUSHUIQING
          ...
          v-if="details.info_left.itype=='shuikushuiqing'"
        ></DetailSHUIKUSHUIQING>
        <!-- 河道水情 -->
        <DetailHEDAOSHUIQING
          :info="details.info_left"
          v-if="details.info_left.itype=='hedaoshuiqing'"
        ></DetailHEDAOSHUIQING>
        <!-- 闸阀水情 -->
        <DetailZHAFASHUIQING
          :info="details.info_left"
        ...
        <DetailSHUIWEIKURONG
          :info="details.info_left"
          v-if="details.info_left.itype=='shuiweikurong'"
        ></DetailSHUIWEIKURONG>
        <!--站点预警详情-->
        <WarmSITEINFO :info="details.info_left" v-if="details.info_left.itype=='yujingxiangqing'"></WarmSITEINFO>
        <!--发送短信详情-->
        <WarmBOXINFO :info="details.info_left" v-if="details.info_left.itype=='fasongduanxin'"></WarmBOXINFO>
      </Drawer>
    </div>
    ...
    <!-- 图片放大模态框 -->
    <div id="imgModal" class="imgModal" ref="imgModal" style="display:none;">
      <!-- 关闭按钮 -->
    ...
    <transition name="Messag">
      <div v-if="!Message_Window_Controller.show" id="Message_Window" key="Window" style="display:none;">
        <div class="title">
          <span>消息栏</span>
...
//站点预警详细信息框
import WarmSITEINFO from "@/warm/warm-detail/Detail-siteinfo.vue";
//发送短信详细信息框
import WarmBOXINFO from "@/warm/warm-detail/Detail-boxsending.vue";
      ...
      this.axios.get("/static/demo-hunanguanqu/ditu.json").then(res => {
        ...
        if (list.length <= 0) return;
          ...
          list.forEach((val, index) => {
                ...
                case "Polygon":
                  symbol = new esri.symbol.SimpleFillSymbol(
                    ...
                    new esri.symbol.SimpleLineSymbol(
                      esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                      new esri.Color([112, 115, 116, 0.8]),
                      1
                    ),
            ...
            this.tu_ceng_list.push({name:val.name, show:true, layer:graphicsSHPlayer});
          });
      ...
      for (let i = 0; i < FeatureLayers.length; i++) {
        ...
        this.getFeatrueLayerWithoutPreconditions(FeatureLayers[i].url, res => {
          ...
          if (
            ...
            featureLayerOBJ.itype === "hedaoshuiqing" ||
    ...
    showDrawer(evt, v) {
      ...
      if (this.details.drawer_show_left) {
        if (this.details.info_left.STCD == graphic.attributes.STCD) {
          //站点预警
          if(v.itype == "yujingxiangqing"){
              if (this.details.info_left.TMSTR == graphic.attributes.TMSTR){
                   // 如果打开的站点和时间相同
                   return this.$Message.warning("该站点详情已打开");
              }
          }else if(this.details.info_left.itype==v.itype){
          // 如果打开的站点相同
          return this.$Message.warning("该站点详情已打开");
          }
        }
